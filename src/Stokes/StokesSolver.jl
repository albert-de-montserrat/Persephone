...
@inline function _fill_R_J!(J_th,J_r,VCOORD_th,VCOORD_r,
    ...
    @turbo for i in 1:length(R_31)
        ...
        R_21[i]    =  VCOORD_r[i, 2]  - VCOORD_r[i, 1] # = -detJa_PL*deta_dth
        R_31[i]    =  VCOORD_r[i, 3]  - VCOORD_r[i, 1] # =  detJa_PL*dxi_dth
        Th_31[i]   = VCOORD_th[i, 3] - VCOORD_th[i, 1] # = -detJa_PL*dxi_dr
        Th_21[i]   = VCOORD_th[i, 2] - VCOORD_th[i, 1] # =  detJa_PL*deta_dr
    end
